...
First-party documentation-only assets created for this repository, such as
README images, are not artifacts for this policy unless they are generated
from, derived from, or vendored from a third-party source, or unless they are
used by the runtime, package contents, release artifacts, or provisioning path.
...
- **First-party documentation-only asset**: A repository-owned file created for
  project documentation, not derived from a third-party source and not consumed
  by runtime code, packaging outputs, release archives, or provisioning paths.
...
4. Every in-scope non-npm artifact must be declared in `artifacts.json`.
   First-party documentation-only assets are out of scope. A system-bundled
...
- npm dependencies are checked through `package-lock.json` and registry release
  timestamps.
- In-scope non-npm artifacts are checked through `artifacts.json` and the
  release catalog implementation for their source.
